const menu = [
  ...
  let search_item = document.getElementById("search-input");
  let nav_link = document.querySelectorAll(".nav-link");
  
  // navbar navigation activation
  ...
  function onloadFunc() {
    ...
    menu.map((item, index) => {
      items.innerHTML += `<div class="col-12 col-md-6 col-lg-4 ">
  ...
  all.addEventListener("click", onloadFunc);
  ...
  starters.addEventListener("click", () => {
    ...
    menu
      ...
      .map((item) => {
        items.innerHTML += `<div class="menu-item col-12 col-md-6 col-lg-4 h-auto">
        <div class="card shadow">
          <img src="${item.img}" class="card-img-top" alt="${item.title}">
          <div class="card-body">
            <h5 class="card-title">${item.title}</h5>
  ...
  const searchText = () => {
    const search = search_item.value.toLowerCase();
